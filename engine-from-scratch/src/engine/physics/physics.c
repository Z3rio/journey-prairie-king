...
void aabb_penetration_vector(float *r, AABB aabb) {
  vec2 min, max;
...
Hit ray_intersect_aabb(float *position, float *magnitude, AABB aabb) {
  Hit hit = {0};
  vec2 min, max;
  aabb_min_max(min, max, aabb);
...
static Hit sweep_static_bodies(AABB aabb, vec2 velocity) {
  ...
  for (u32 i = 0; i < state.static_body_list->len; ++i) {
    ...
    if (hit.time < result.time) {
      ...
      (
        (fabsf(velocity[0]) > fabsf(velocity[1]) && hit.normal[0] != 0) ||
...
usize physics_static_body_create(float *position, float *size) {
...
usize physics_body_create(float *position, float *size) {
  Body body = {
    .aabb = {
      .position = { position[0], position[1] },
      .half_size = { size[0] * 0.5, size[1] * 0.5 },
